"""
This script compares the propd_combined DE results to the Wilcoxon test DE results
"""
# ----------------------------
# Import libraries
# ----------------------------
...
#TARGET_GENE = "SAFB" # for testing only
...
def compute_theta_enrichment(theta_df: pd.DataFrame, ascending: bool = True):
    ...
    # near zero -> neutral
    balance = np.abs(np.abs(es_pos) - np.abs(es_neg))
    strong  = np.maximum(np.abs(es_pos), np.abs(es_neg))

    middle_flag = (balance < 0.05) & (strong > 0.1)  # tweak 0.05 and 0.1 to your taste

    direction = np.where(middle_flag, "middle",
             np.where(es > 0, "top", np.where(es < 0, "bottom", "neutral")))
...
def main():
    ...
    # Remove rows in which direction is middle
    #es_df = es_df[es_df["direction"] != "middle"].copy()
    #print(f"enrichment df after removing middle: {es_df.shape[0]} genes")
    ...
    merged_comb_wilcox = pd.merge(propd_comb_de, wilcoxon_de[["feature", "fold_change", "p_value", "fdr", "significant"]], left_on="gene_name", right_on="feature", how="inner")

    # Add a column with the cumulative sum of significant genes (True) in wilcoxon_de_sig
    merged_comb_wilcox["cum_significant_theta_comb"] = merged_comb_wilcox["significant"].cumsum()
    ...
    # Merge merged_comb_wilcox with propd_single_de on gene_name (left join)
    merged_all = pd.merge(merged_comb_wilcox, propd_single_de[["gene_name", "weighted_connectivity", "ranked_weighted_connectivity"]], on="gene_name", how="left")
    ...
    es_df = es_df.sort_values(by="es", ascending=False).reset_index(drop=True)
    es_df["es_rank"] = np.arange(1, es_df.shape[0] + 1)
    merged_all = pd.merge(merged_all, es_df[["gene_name", "es_rank"]], on="gene_name", how="left")
    merged_all = merged_all.sort_values(by="es_rank", ascending=True).reset_index(drop=True)
    merged_all["cum_significant_theta_es"] = merged_all["significant"].cumsum()
    ...
    plt.plot(merged_all["es_abs_rank"], merged_all["cum_significant_theta_es_abs"], label="propd_combined_enrichment_abs", color="green")
    ...
    merged_all = merged_all.sort_values(by="p_value", ascending=True).reset_index(drop=True)
    merged_all["p_value_rank"] = np.arange(1, merged_all.shape[0] + 1)
    merged_all["cum_significant_p_value"] = merged_all["significant"].cumsum()
    print("Ranked by p-value:")
    print(merged_all[["p_value","p_value_rank", "cum_significant_p_value"]].head(10))
    plt.plot(merged_all["p_value_rank"], merged_all["cum_significant_p_value"], label="wilcoxon_p_value", color="black", linestyle="--")

    plt.legend()
    plt.xlabel("Rank")
    plt.ylabel("Cumulative sum of significant genes (wilcoxon fdr < 0.05)")
    plt.title(f"Cumulative sum of Wilcoxon significant genes vs Rank for {TARGET_GENE}")

    plt.savefig(os.path.join(OUTPUT_DIR, f"cumulative_significant_genes_{TARGET_GENE}.png"))    

    merged_all = merged_all.sort_values(by="es_rank", ascending=True).reset_index(drop=True)
    print("Ranked by enrichment score (es):")
    print(merged_all[["es","es_rank", "cum_significant_theta_es"]].head(10))
    plt.plot(merged_all["es_rank"], merged_all["cum_significant_theta_es"], label="propd_combined_enrichment_sign", color="purple")


    merged_all = merged_all.sort_values(by="es_pos", ascending=False).reset_index(drop=True)
    merged_all["es_pos_rank"] = np.arange(1, merged_all.shape[0] + 1)
    merged_all["cum_significant_theta_es_pos"] = merged_all["significant"].cumsum()
    print("Ranked by enrichment score (es_pos):")
    print(merged_all[["es_pos","es_neg", "cum_significant_theta_es"]].head(10))
    plt.plot(merged_all["es_pos_rank"], merged_all["cum_significant_theta_es_pos"], label="propd_combined_enrichment_pos", color="brown")

    merged_all = merged_all.sort_values(by="es_neg", ascending=True).reset_index(drop=True)
    merged_all["es_neg_rank"] = np.arange(1, merged_all.shape[0] + 1)
    merged_all["cum_significant_theta_es_neg"] = merged_all["significant"].cumsum()
    print("Ranked by enrichment score (es_neg):")
    print(merged_all[["es_neg","es_neg_rank", "cum_significant_theta_es_neg"]].head(10))
    plt.plot(merged_all["es_neg_rank"], merged_all["cum_significant_theta_es_neg"], label="propd_combined_enrichment_neg", color="pink")
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(OUTPUT_DIR, f"cumulative_significant_genes_{TARGET_GENE}_nomiddle.png"))
    plt.close() 

    # # Save merged_all to csv
    merged_all.to_csv(os.path.join(OUTPUT_DIR, f"merged_all_{TARGET_GENE}.csv"), index=False)
    ...
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(OUTPUT_DIR, f"theta_combined_vs_rank_theta_comb_colored_{TARGET_GENE}.png"))
