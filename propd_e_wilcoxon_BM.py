...
PROPD_SINGLE_PATH = f"/users/cn/caraiz/propr_new/results/propd_single/theta_emergent/genewise_metrics/pert_{TARGET_GENE}_genewise_metrics.csv"
...
OUTPUT_DIR = f"/users/cn/caraiz/propr_new/results/propd_benchmark/{TARGET_GENE}/theta_emergent/"
...
def main():
    ...
    propd_single_de = propd_single_de.sort_values(by="weighted_connectivity_e", ascending=False).reset_index(drop=True)
    # Add a column named 'ranked_weighted_connectivity' with the rank (1-based)
    propd_single_de["ranked_weighted_connectivity_e"] = np.arange(1, propd_single_de.shape[0] + 1)

    # Merge merged_comb_wilcox with propd_single_de on gene_name (left join)
    merged_all = pd.merge(merged_comb_wilcox, propd_single_de[["gene_name", "weighted_connectivity_e", "ranked_weighted_connectivity_e"]], on="gene_name", how="left")
    # Rank merged_all by ranked_weighted_connectivity_e (ascending)
    merged_all = merged_all.sort_values(by="ranked_weighted_connectivity_e", ascending=True).reset_index(drop=True)
    # Add a column named 'cum_significant_weighted_connectivity_e' with the cumulative sum of significant genes (True) in wilcoxon_de_sig
    merged_all["cum_significant_weighted_connectivity_e"] = merged_all["significant"].cumsum()
    ...
    print(merged_all[["weighted_connectivity_e","ranked_weighted_connectivity_e", "cum_significant_weighted_connectivity_e"]].head(10))
    plt.plot(merged_all["ranked_weighted_connectivity_e"], merged_all["cum_significant_weighted_connectivity_e"], label="propd_single", color="orange")
