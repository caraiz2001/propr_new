...
WILCOXON_PATH = f"/users/cn/caraiz/VCC/de_corrected_nn_results_per_gene/{TARGET_GENE}_de_genes.tsv"
...
def main():
    ...
    plt.xlabel("Rank")
    plt.ylabel("Cumulative sum of significant genes (wilcoxon corrected fdr < 0.05)")
    plt.title(f"Cumulative sum of Wilcoxon (corrected) significant genes vs Rank for {TARGET_GENE}")
    plt.legend()
    plt.grid()
    plt.savefig(os.path.join(OUTPUT_DIR, f"corrected_cumulative_significant_genes_{TARGET_GENE}_sign.png"))
    plt.close() 
